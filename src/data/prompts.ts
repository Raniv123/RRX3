...
export function buildAIPrompt(
  ...
  // אנטי-חזרה — חילוץ דפוסים שכבר נאמרו בשיחה (טקסט + פתיחות + 3 מילים ראשונות)
  const recentTexts = messages.slice(-12).map(m => m.text || '').filter(Boolean);
  const recentOpenings = Array.from(new Set(recentTexts.map(t => t.split(/\s+/).slice(0, 3).join(' ')).filter(s => s.length > 5)));
  const antiRepeatBlock = recentTexts.length > 3 ? `
## ⛔ אסור לחזור — דפוסים מהשיחה הנוכחית:

**ביטויים שכבר נאמרו:**
${recentTexts.slice(-8).map(t => `- "${t.slice(0, 70)}"`).join('\n')}

**פתיחות שכבר השתמשנו בהן (אסור לפתוח כך שוב):**
${recentOpenings.slice(-6).map(o => `- "${o}..."`).join('\n')}

**חוקי שונות:**
1. כל chip חייב להיות חדש בנימה, בניסוח, ובזווית רגשית
2. אל תפתח משפט באותן 3 מילים ראשונות שכבר השתמשנו בהן
3. אל תחזור על אותו דימוי/מטאפורה
4. אם הצורה האחרונה הייתה ישירה — עכשיו נסיון לרמיזה. אם הייתה פגיעה — עכשיו ביטחון. אם הייתה ארוכה — עכשיו קצרה
` : '';
...
- actionTip: פעולה פיזית/אינטימית **אחת בלבד** — קונקרטית, מדורגת לשלב ולמתח:
  ...
  ${phase === 'HOT' ? `HOT → יד על ירך ${isMAN ? 'שלה' : 'שלו'} או על הצוואר, נשיפה קלה, ליקוק שפתיים בעיניים, נשיקה קלה` : ''}
  ...
  ${phase === 'FIRE' && tension >= 81 && tension < 87 ? `FIRE-ORAL-TEASE → ליקוק עיגולים מסביב — ${isMAN ? 'לפות שלה' : 'לזין שלו'}, בלי לגעת ישירות` : ''}
  ${phase === 'FIRE' && tension >= 87 && tension < 93 ? `FIRE-ORAL-FULL → ${isMAN ? 'ליקוק עמוק של הפות שלה — לאט, עיגולים, ועומק' : 'לקיחת הזין שלו בפה — לאט, עומק, ושפתיים סוגרות'}` : ''}
...
**אסור בהחלט:** צורות כפולות עם סלאש (כמו "שאתה/את", "אמר/ת", "רוצה/ה", "יכול/ה"). בחר תמיד צורה אחת לפי מין השותף/השותפה.
...
- גוף שני (${isMAN ? 'השותפה' : 'השותף'} — ${isMAN ? 'האישה' : 'הגבר'}): ${isMAN ? '"שאת לא אומרת", "כשאת מדברת", "את גורמת לי"' : '"שאתה לא אומר", "כשאתה מדבר", "אתה גורם לי"'}
